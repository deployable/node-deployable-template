...
module.exports = {
  ...
  builder: yargs => {
    return yargs
    .usage('Usage: $0 build <template_name|path>')
    .options({
  ...
  handler: argv => {
    ...
    return ts.go().then(res => {
        console.log('Template command finished:\n\n Built "%s" in "%s"\n', argv.template, ts.output_path)
        debug('res', res)
      })
      .catch(error => {
        ...
        if ( argv.debug || process.env.DEBUG ) {
          console.error('DEBUG: %s', error.stack)
          if (error.original) console.error('DEBUG: %s', error.original.stack)
        }

        // Normal output

        console.error('Template command failed:\n\n %s (%s)', error.message, error.name )
        if ( error.description ) console.error('\n %s', error.description)
        
        // Trim stack for some foff
        let app_root = path.resolve(__dirname, '..') + path.sep
        debug('App root to remove from stack', app_root, __dirname)
