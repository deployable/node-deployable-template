...
//import {ExtendedError} from 'deployable-errors'
...
const ConfigFileError = module.exports.ConfigFileError = class ConfigFileError extends ExtendedError {
...
module.exports.ConfigFile = class ConfigFile {
  ...
  generateFile(){
    switch ( this.type ) {
      ...
      default: return this.generateConfigFile().join(this.line_ending)
    }
  }
  ...
  writeFile () {
    return new Promise((resolve, reject) => {
      let contents = null
      switch ( this.type ) {
        case 'yaml': contents = YAML.dump(this.store); break
        case 'json': contents = JSON.stringify(this.store); break
        default: contents = this.generateFile().join(this.line_ending)
      }
      return resolve( fs.writeFile(this.path, mode, contents) )
    })
    .then(res => {
      debug('file written to', this.path, res)
    })
  }
